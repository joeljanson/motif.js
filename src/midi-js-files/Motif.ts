...
class Motif {
	...
	private _notes: number[]; // Private property to store the MIDI notes
	rythm: number[]; // An array containing information about the time and note indexes (corrected spelling)
	motifId: string; // A variable to identify the motif
	stepHarmonization: number[][]; // An array containing information about harmonization
	position: string | number; // Information about the offset of the motif
	transposition?: number; // Transpose the motif in semitones (optional)
	submotifs: Motif[]; // An array holding all submotifs

	constructor(options: Partial<Motif>) {
		this._notes = [];
		if (Array.isArray(options.notes)) {
			// If it's an array, directly assign it to this.notes
			this.setNotes(options.notes);
		} else {
			// If it's not an array, initialize this.notes as an empty array
			console.error(
		...
		this.rythm = options?.rythm ?? this.notes.map((note, index) => index); // Corrected spelling
	 ...
	 */
	// Define a getter for notes
	get notes(): Array<number | string> {
		return this._notes;
	}
	// Define a setter for notes
	set notes(newNotes: Array<number | string>) {
		// Use the setNotes method to handle the conversion and assignment
		this.setNotes(newNotes);
	}

	setNotes(notes: (number | string)[]) {
		console.warn("Turning all values into MIDI. Is this the desired behavior?");

		// Create an array to store the converted MIDI notes
		const midiNotes = [];

		for (const noteName of notes) {
			if (typeof noteName === "string") {
				// Try to convert noteName to MIDI value
				const midiNote: number = Frequency(noteName).toMidi();

				if (!isNaN(midiNote)) {
					// Conversion successful, add it to the MIDI notes array
				...
				// It's already a number (assumed to be MIDI value), so add it directly
				midiNotes.push(noteName);
			} else {
				console.error(
					"Invalid note type. Note must be a string (note name) or a number (MIDI value). Note was: ",
					noteName
				);
			}
		}
		// Assign the MIDI notes array to this._notes
