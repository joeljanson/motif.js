...
const RythmicPatchTwo: React.FC = () => {
	...
	const setupMotifs = () => {
		...
		const firstMotif = new Motif({
			...
			noteIndexes: [0, 1, 1, 2, 3],
			transpositions: [-1, -2, -3, -4],
			//velocities: [1, 1, 1],
			//length: Time("2m").toSeconds(),
		});
		firstMotif.setNoteNames(["C4", "Db4", "D4", "B3"]);
		...
		secondMotif.setNoteNames(["G3", "G3", "G3"]);
		secondMotif.position = Time("8n").toSeconds();
		...
		motifs.current.push(firstMotif, secondMotif, fourth);
...
export default RythmicPatchTwo;
